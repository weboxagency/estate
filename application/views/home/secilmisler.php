<main>
  <div class="page-container container pb-0">
    <div class="announcement-group__header pt-3 justify-content-start">
      <h1 class="announcement-title">Seçilmişlər</h1>
      <div class="announcement-filter filter-primary ml-lg-3">
        <span class="link-button" x-wishlist_count=""><?= $count ?></span>
        <span>Elan</span>
      </div>
    </div>
    <div class="announcement-group__body">
      <?php if (!empty($secilmisler)){ ?>
        <?php foreach ($secilmisler as $value) { ?>
      <div x-remove-announcement="<?= $value['id'] ?>" class="announcement announcement--short announcement-template" x-announcement-owner="438">
    ...
    <?php }else{ ?>
        ...
        <div class="container">
        <div class="notfound">
        <h1>Seçilmiş elanınız mövcud deyil!</h1>
        </div>
        </div>
    <?php } ?>
    </div>

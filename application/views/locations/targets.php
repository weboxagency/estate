	...
	<div class="tabs-custom">
		<ul class="nav nav-tabs">
			<li class="<?=(empty($validation_error) ? 'active' : '') ?>">
				<a href="#list" data-toggle="tab"><i class="fas fa-list-ul"></i> <?=translate('all_targets')?></a>
			</li>
			<li class="<?=(!empty($validation_error) ? 'active' : '') ?>">
				<a href="#create" data-toggle="tab"><i class="far fa-edit"></i> <?=translate('add_target')?></a>
			</li>
		</ul>
					...
					<table class="table table-bordered table-hover table-condensed mb-none table-export">
						...
						<tbody>
							...
							<tr>
								...
								<td>
									<?php if (get_permission('targets', 'is_edit')) { ?>
										<div class="material-switch ml-xs">
											<input class="targets-switch" id="switch_<?=$row['id']?>" data-id="<?=$row['id']?>" name="targets_switch<?=$row['id']?>" 
								...
								<td class="min-w-c">
									<!--update link-->
									<a href="<?=base_url('locations/target_edit/'.$row['id'])?>" class="btn btn-default btn-circle icon">
										<i class="fas fa-pen-nib"></i>
									</a>
									<!-- delete link -->
									<?php echo btn_delete('locations/target_delete/' . $row['id']);?>
								</td>
							</tr>
							<?php endforeach; ?>
						</tbody>
			...
			<div class="tab-pane <?=(!empty($validation_error) ? 'active' : '')?>" id="create">
				<?php echo form_open($this->uri->uri_string(), array('class' => 'form-horizontal form-bordered validate')); ?>
					...
					<div class="form-group mt-md">
						...
						<div class="col-md-6">
							<input type="text" class="form-control" name="target_name" value="<?=set_value('target_name')?>" />
